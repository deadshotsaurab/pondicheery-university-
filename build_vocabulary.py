...
def load_pipeline_modules():
    import config
    from feature_engineering import tokenize_text, filter_words, extract_all_features, get_feature_matrix, normalize_features
    from model_training import train_gmm_model, predict_components, predict_probabilities, analyze_components, create_vocabulary_datasets
    return config, tokenize_text, filter_words, extract_all_features, get_feature_matrix, normalize_features, \
           train_gmm_model, predict_components, predict_probabilities, analyze_components, create_vocabulary_datasets


def run_pipeline(uploaded_files, n_components=3):
    config, tokenize_text, filter_words, extract_all_features, \
    get_feature_matrix, normalize_features, train_gmm_model, \
    predict_components, predict_probabilities, analyze_components, \
    create_vocabulary_datasets = load_pipeline_modules()

    # Reset file pointers and read docs
    ...
    for uf in uploaded_files:
        ...
        docs.append(text)
        with open(os.path.join(tmpdir, uf.name), "w", encoding="utf-8") as f:
            f.write(text)

    total_chars = sum(len(d) for d in docs)
    if total_chars < 100:
        raise ValueError(
            "Uploaded file(s) are too small (< 100 characters). "
            "Please upload real documents with at least a few paragraphs."
        )

    # Build corpus
    all_words, doc_freq_counter = [], Counter()
    for doc in docs:
        tokens = tokenize_text(doc)
        all_words.extend(tokens)
        doc_freq_counter.update(set(tokens))
    word_freq    = Counter(all_words)
    raw_vocab    = len(word_freq)
    total_tokens = sum(word_freq.values())

    # Adaptive filtering: relax thresholds for small corpora
    orig_min_freq = config.MIN_FREQUENCY
    orig_min_len  = config.MIN_WORD_LENGTH
    if total_tokens < 500:
        config.MIN_FREQUENCY   = 1
        config.MIN_WORD_LENGTH = 2
    elif total_tokens < 2000:
        config.MIN_FREQUENCY   = max(1, config.MIN_FREQUENCY - 1)
        config.MIN_WORD_LENGTH = max(2, config.MIN_WORD_LENGTH - 1)

    word_freq = filter_words(word_freq, documents=docs)

    config.MIN_FREQUENCY   = orig_min_freq   # restore
    config.MIN_WORD_LENGTH = orig_min_len

    if len(word_freq) < n_components * 3:
        raise ValueError(
            f"{len(word_freq)} words survived filtering — need at least {n_components * 3} "
            f"for {n_components} clusters.\n\n"
            f"Your corpus has {total_tokens} tokens from {raw_vocab} unique words. "
            "Please upload larger .txt files (ideally 1000+ words each)."
        )

    # Features
    df             = extract_all_features(list(word_freq.keys()), word_freq, docs, doc_freq_counter)
    feature_matrix = get_feature_matrix(df, config.FEATURES)
    if feature_matrix.shape[0] == 0:
        raise ValueError("Feature matrix is empty. Please use larger input files.")

    normalized, scaler = normalize_features(feature_matrix, config.SCALING_METHOD)

    # Train GMM
    gmm        = train_gmm_model(normalized)
    labels     = predict_components(gmm, normalized)
    probs      = predict_probabilities(gmm, normalized)
    comp_stats = analyze_components(gmm, normalized, df, labels)

    # Label assignment — zipf-primary (highest zipf = most common = LAYMAN)
    sorted_cids = sorted(comp_stats.keys(),
                         key=lambda c: comp_stats[c].get("avg_zipf_score", 0),
                         reverse=True)
    label_names = ["LAYMAN", "STUDENT", "PROFESSIONAL"]
    label_map   = {cid: label_names[i] for i, cid in enumerate(sorted_cids)
                   if i < len(label_names)}

    datasets = create_vocabulary_datasets(df, labels, probs, label_map)

    # Normalize: ensure every dataset value is a DataFrame
    normalized_datasets = {}
    for lbl, data in datasets.items():
        if isinstance(data, pd.DataFrame):
            normalized_datasets[lbl] = data
        elif isinstance(data, list):
            if len(data) == 0:
                normalized_datasets[lbl] = pd.DataFrame(columns=["word", "confidence"])
            elif isinstance(data[0], dict):
                normalized_datasets[lbl] = pd.DataFrame(data)
            else:
                # list of plain word strings
                normalized_datasets[lbl] = pd.DataFrame({"word": data, "confidence": 1.0})
    ...
    return normalized, df, labels, probs, comp_stats, label_map, datasets, gmm
...
def chart_pca(normalized, labels, label_map):
    ...
    for cid, lbl in label_map.items():
        ...
        ell = Ellipse(xy=mean, width=w, height=h, angle=angle,
                      color=COLORS[lbl], alpha=0.12, zorder=0)
        ell_edge = Ellipse(xy=mean, width=w, height=h, angle=angle,
                           fill=False, edgecolor=COLORS[lbl],
                           linewidth=1.5, alpha=0.6, zorder=1)
        ax.add_patch(ell)
        ax.add_patch(ell_edge)
        ax.plot(*mean, "+", color=COLORS[lbl], markersize=10, markeredgewidth=2)
